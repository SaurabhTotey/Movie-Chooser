		...
		<>
			...
			<main>
				...
				<div className="prose lg:prose-xl mx-auto mt-12">
					...
					<p>
						This page is a wall of useless text that no one will read. Use the navigation above to go to where you need
						to go.
					</p>
					<h3>General</h3>
					<p>
						This is a site for choosing movies to watch when watching movies as a group. Oftentimes, selecting movies in
						a group setting can be difficult: different people want to watch different movies. This site allows users to
					...
					<p>
						...
						. Statistics are purely for fun and are based off of movie ratings (so rate movies for fun statistics
						maybe).
					</p>
					<h3>Accounts</h3>
					<p>
					...
					<p>
						In terms of security, this website utilizes the piss-poor strategy of hoping all the users act in good
						faith. This is due to the assumption that this will be hosted locally and only a specific handful of people
						will use this. Do not put sensitive information into this website. Passwords are hashed, but caution should
						still be exercised. Please don&apos;t game what this website allows since it is based on trust.
					</p>
					<h3>Movie Lists</h3>
					<p>
						Each account has two movie lists: a to-watch list and an already-watched list. The to-watch list of a user
						is the list of movies that can be selected on the party page. The already-watched list is the list of movies
						that the user has already seen in a party. Both lists for a user are visible on their{" "}
						<Link href="./profile">
							<a>profile page</a>
						</Link>
					...
					<p>
						...
						chance of being chosen. Each movie can only appear in a user&apos;s to-watch list once. A higher weight for
						a movie means a higher chance of that movie being chosen if the user&apos;s list is chosen. Weights are
						relative to the weights of other entries in a to-watch list. Movies can be added to the signed-in
						user&apos;s to-watch list on the{" "}
						<Link href="./add-movie">
							<a>add-movie page</a>
						</Link>
					...
					<p>
						...
						Movies can make duplicate appearances as users can watch movies multiple times. The user can add their
						rating or change their rating for a watched movie on their{" "}
						<Link href="./profile">
							<a>profile page</a>
						</Link>
					...
					<p>
						...
						, which requires the user to be signed in. The user then selects all users who will be watching movies.
						After that, the user is free to let the site choose a random movie. After the site chooses a movie, the user
						can mark the movie as watched for all users who were marked as watching at the time that the movie was
						chosen. In that case, any users who have the movie marked as watched will also have the movie removed from
						their to-watch list.
					</p>
					<p>
						The algorithm for choosing a movie is as follows. Every user who is watching and has a non-empty to-watch
						list with total weight greater than 0 has an equal chance of their to-watch list being picked. Once a list
						is picked, the list&apos;s weightings are used to determine how likely each movie is. A movie is then chosen
						according to this likelihood. The intricacies of weights are described in the next section.
					</p>
					<h3>Weights</h3>
					<p>
						Weights apply solely to movies in to-watch lists, and are relative to other weights within the same list.
						For a movie, <span>{"\\(i\\)"}</span>, with weight <span>{"\\(w_i\\)"}</span>, it&apos;s probability of
						being chosen, <span>{"\\(\\mathcal{P}(i)\\)"}</span>, is{" "}
						<span>
							{
								"\\[\\mathcal{P}(i) = \\begin{cases} \\frac{w_i}{\\sum_j w_j} &\\qquad \\sum_j w_j \\neq 0 \\\\ 0 &\\qquad \\text{otherwise} \\end{cases}\\]"
							}
						</span>
						where <span>{"\\(\\sum_j w_j\\)"}</span> is the total sum of the weight for a single to-watch list. The
						chance of any specific to-watch list being used is equal for each user whose total to-watch list weight is
						greater than 0. If a user&apos;s to-watch list total weight comes out to 0, their list will not be chosen.
					</p>
					<p>
